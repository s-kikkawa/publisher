...
func main() {
    ...
    go func(){
        ...
        router.POST("/insert", func(ctx *gin.Context) {
            ...
            message.SendMessage("INSERT", fmt.Sprint(id), ctx.PostForm("itemCode"), ctx.PostForm("text"))
        ...
        router.POST("/update/:id", func(ctx *gin.Context) {
            ...
            message.SendMessage("UPDATE", ctx.Param("id"), ctx.PostForm("itemCode"), ctx.PostForm("text"))
            ctx.Redirect(http.StatusFound, "/")
        })
        // 削除
        router.POST("/delete/:id", func(ctx *gin.Context) {
            database.Delete(convertId(ctx.Param("id")))
            message.SendMessage("DELETE", ctx.Param("id"), ctx.PostForm("itemCode"), ctx.PostForm("text"))
            ctx.Redirect(http.StatusFound, "/")
        })
